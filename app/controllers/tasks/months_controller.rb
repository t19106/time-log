class Tasks::MonthsController < ApplicationController
  ...
  def index
    ...
    _tasks = Task.where(user: current_user, start_at: _date.beginning_of_day.._date.end_of_day).or(Task.where(user: current_user, end_at: _date.beginning_of_day.._date.end_of_day)).map { |task| task.adjust_overnight_range(_date) }
